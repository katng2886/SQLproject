Practice problem subqueries and merge
...
COMMIT;
      ...
      /*NGOC (Katie) NGUYEN
Add a new row in the orders table with the following data: Order#= 1021, Customer#=1009, Orderdate = July 20, 2009. */
INSERT INTO ORDERS (
     ORDER#,
     CUSTOMER#,
     ORDERDATE
...
--   Ngoc (Katie) NGUYEN
--  1. List the book title and retrail price for all books with a retail price lower than the avg retail price of all books sold by Justlee books
   SELECT
     TITLE,
     RETAIL
   FROM
...
--   Ngoc (Katie) NGUYEN
--  2. Determine which books cost less than avg cost of other books in the same CATEGORY
   SELECT
     B.TITLE,
     B.CATEGORY
   FROM
...
--   Ngoc (Katie) Nguyen
--  5. Determine which author or authors wrote the books most frequently purchased by customers of JustLee
     Write an SQL query that uses a multiple-row subquery in a WHERE clause. The subquery will include the GROUP BY statement and another multiple-row subquery in a HAVING clause.
...
-- Ngoc (Katie) Nguyen
-- 6. List the title of all books in the same category as books, previously purchaes by customer 1007. Dont include books this customer has already purchased.
-- 6.	To complete problem 6, write an SQL query that uses two multiple-row subqueries in a WHERE clause joined with the AND operator. Each subquery will join multiple tables.
     Ngoc (Katie) Nguyen
   SELECT
     TITLE
   FROM
...
-- 7.	To complete problem 7, write an SQL query that uses a single-row subquery in a WHERE clause.
   SELECT
     SHIPCITY,
     SHIPSTATE,
     SHIPDATE,
...
-- Ngoc (Katie) Nguyen
-- 8. Determine which customers placed orders for the least expensive book (in term of regular price)
-- 8.	To complete problem 8, write an SQL query that joins four tables and uses a single-row subquery in a WHERE clause.
   SELECT
     C.CUSTOMER#
   FROM
     CUSTOMERS  C
...
-- Ngoc (Katie) Nguyen
-- 10. Determine which books were published by the publisder of The Wok Way to Cook.

   SELECT
     B.TITLE "BOOKS have the same Publisher as THE WOK WAY TO COOK",
     PU.NAME
   FROM
     BOOKS     B
     JOIN PUBLISHER PU
