...
-- NGOC (Katie) NGUYEN
--1. Retrive record from one of the tables in JustLee Books db. Include comparsion operators.
SELECT * FROM customers WHERE region = 'NW';

-- NGOC (Katie) NGUYEN
--2. Include between ... and operators
SELECT * FROM orders WHERE shipcost between 4 and 6;

-- NGOC (Katie) NGUYEN
--3. Include IN operator. 
SELECT PubID, ISBN, Title, PubDate FROM books WHERE PubID IN (1,2,3) ORDER BY PubId ASC;

-- NGOC (Katie) NGUYEN
--4. Include the LIKE operator with either % or _ (or both).
SELECT * FROM author WHERE lname LIKE 'W%';
...
-- NGOC (Katie) NGUYEN
--6. Include multiple conditions using logical operators (AND and OR).
SELECT * FROM customers WHERE state = 'FL' AND region = 'SE';


-- NGOC (Katie) NGUYEN
--7. Use logical operators to join multiple conditions that include at least one of the arithmetic operators and one of the special operators. Use the ORDER BY statement to order the output.
SELECT * FROM books WHERE cost > 20 AND DISCOUNT IS NOT NULL ORDER BY cost ASC;
